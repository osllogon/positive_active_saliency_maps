...
def main() -> None:
    ...
    if generate_graphs:
        ...
        if not os.path.exists(f"{DATA_PATH[dataset]}/saved/{model_type}_{pretrained}"):
            ...
            progress_bar = tqdm(
                range(
                    (len(train_data) + len(val_data)) * len(METHODS) * len(PERCENTAGES)
                )
            )
            ...
            for loader_name, loader in {"train": train_data, "val": val_data}.items():
                ...
                for images, labels in loader:
                    ...
                    if not os.path.exists(
                        f"{DATA_PATH[dataset]}/saved/{model_type}_{pretrained}/{loader_name}/labels"
                    ):
                        os.makedirs(
                            f"{DATA_PATH[dataset]}/saved/{model_type}_{pretrained}/{loader_name}/labels"
                        )
                    if not os.path.exists(
                        f"{DATA_PATH[dataset]}/saved/{model_type}_{pretrained}/"
                        f"{loader_name}/outputs"
                    ):
                        os.makedirs(
                            f"{DATA_PATH[dataset]}/saved/{model_type}_{pretrained}/{loader_name}/outputs"
                        )
                        ...
                        for percentage in PERCENTAGES:
                            ...
                            for subs_value in [0, 1]:
                                ...
                                if not os.path.exists(
                                    ...
                                    os.makedirs(
                                        f"{DATA_PATH[dataset]}/saved/{model_type}_{pretrained}/{loader_name}/"
        ...
        for subs_value in [0, 1]:
            for loader_name in ["train", "val"]:
                ...
                for method_name, method in METHODS.items():
                    ...
                    if subs_value == 0 and (
                        method_name == "negative_saliency_map"
                        or method_name == "inactive_saliency_map"
                    ):
                        continue

                    # ignore positive and actives for 1 subs
                    if subs_value == 1 and (
                        method_name == "positive_saliency_map"
                        or method_name == "active_saliency_map"
                    ):
                        continue
                    ...
                    for percentage in PERCENTAGES:
                        ...
                        for file_name in os.listdir(
                            ...
                            outputs = torch.argmax(
                                torch.load(
                                    f"{DATA_PATH[dataset]}/saved/{model_type}_{pretrained}/{loader_name}"
                                    f"/{method_name}/{subs_value}/{percentage}/outputs/{file_name}"
                                ),
                                dim=1,
                            )
                        ...
                        if last_percentage is None:
                            ...
                            auc += (
                                abs(results[method_name][-1] - results[method_name][-2])
                                / 2
                                + min(
                                    results[method_name][-1], results[method_name][-2]
                                )
                            ) * (percentage - last_percentage)
                            last_percentage = percentage

                    print(
                        f"{loader_name} AUC {method_name} for {subs_value}: {auc:.2f}"
                    )

                # check dir
                if not os.path.exists(
                    f"visualizations/graphs/{dataset}/{model_type}_{pretrained}/{loader_name}"
                ):
                    os.makedirs(
                        f"visualizations/graphs/{dataset}/{model_type}_{pretrained}/{loader_name}"
                    )
                ...
                plt.legend(legend)
                plt.savefig(
                    f"visualizations/graphs/{dataset}/{model_type}_{pretrained}/{loader_name}/{subs_value}.pdf",
                    bbox_inches="tight",
                    pad_inches=0,
                    format="pdf",
                )
